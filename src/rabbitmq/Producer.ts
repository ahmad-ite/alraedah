...
class Producer extends AMQBBase {
  ...
  public sendDelayedMessageToQueue(
    queueName: string,
    delayInMills: number,
    data: string
  ): Promise<void> {
    logger.logInfo(
      Producer.LOG_TAG,
      `Initiate Send delayed message to ${queueName} at ${new Date().toTimeString()}`
    );
    ...
    return new Promise((resolve, reject) => {
      this.assertExchange(INTERMEDIATE_EXCHANGE, INTERMEDIATE_EXCHANGE_TYPE)
        ...
        .then((_) =>
          this.bindQueue(INTERMEDIATE_QUEUE, INTERMEDIATE_EXCHANGE, "")
        )
